...
| Top app bar | `AccessibleTopBar` | SCR-002, SCR-003, SCR-005, SCR-006, SCR-007, SCR-008, SCR-009, SCR-010 | title, back visibility, leading label, trailing widget | back-enabled, back-hidden, trailing action | Native `AppBar` with header title and labeled back action. |
...
## Components Deferred To Runtime Modules

| Planned component | Source screens | Reason deferred | Future module notes |
|---|---|---|---|
| `ConnectionDeviceCard` | SCR-006 | No Raspberry Pi discovery, IDs, signal, or pairing is implemented. | Add with local Pi connection state machine. |
...
| Raspberry Pi flow | searching, no devices, found device, pairing, connecting, connected, connection lost, incompatible device | Local wearable flow needs a complete state machine later. |
...
- All implemented components are covered indirectly by clean analysis and the
  128-test suite. Physical TalkBack and platform feedback checks remain open.
