...
class MobileCameraService implements CameraService {
  ...
  bool _isChangingTorch = false;

  @override
  Future<void> setTorch(bool enabled) async {
    if (_isTorchOn == enabled) return;
    final ctrl = _controller;
    if (ctrl == null || !ctrl.value.isInitialized) return;
    if (_isChangingTorch) return;
    _isChangingTorch = true;
    try {
      await ctrl.setFlashMode(
        enabled ? cam.FlashMode.torch : cam.FlashMode.off,
      );
      _isTorchOn = enabled;
    } catch (e) {
      final msg = e.toString().toLowerCase();
      if (!msg.contains('operationcanceled') && !msg.contains('canceled')) {
        _lastError = 'Torch control unavailable: $e';
      }
    } finally {
      _isChangingTorch = false;
    }
  }
  ...
  void _handleControllerUpdate(cam.CameraController controller) {
    ...
    // Transient OperationCanceledException in CameraX occurs when flash or focus
    // updates are superseded. It is not a fatal hardware error.
    if (lower.contains('operationcanceled') ||
        lower.contains('operation cancel')) {
      return;
    }
