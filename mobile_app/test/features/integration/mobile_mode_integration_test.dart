...
void main() {
  ...
  group('Module 14 — Mobile Mode Integration', () {
    ...
    test(
      'UT-INT-022: vibration-only mode auto-enables audio for detection safety',
      () async {
        final fakeCamera = FakeCameraService();
        final fakeInference = FakeInferenceService();
        final container = ProviderContainer(
        ...
        expect(session.state, MobileAssistanceState.active);
        final updatedSettings = container.read(appSettingsControllerProvider);
        expect(
          updatedSettings.feedbackSettings.mode,
          FeedbackMode.audioAndVibration,
        );
        expect(updatedSettings.vibrationEnabled, isTrue);
        expect(fakeCamera.startPreviewCalled, isTrue);
        expect(fakeInference.loadModelCalled, isTrue);
      },
    );
