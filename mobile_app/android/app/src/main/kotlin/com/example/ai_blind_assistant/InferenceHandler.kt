...
class InferenceHandler(private val assetManager: AssetManager) : MethodChannel.MethodCallHandler {
    ...
    private var householdInterpreter: Interpreter? = null
    private var householdOutputShape = intArrayOf()
    private var householdOutputType = DataType.FLOAT32
    private var householdOutputScale = 0.0f
    private var householdOutputZeroPoint = 0
    private var householdOutputBuffer: ByteBuffer? = null

    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            "loadModel" -> {
                val modelAsset = call.argument<String>("modelAsset").orEmpty()
                val householdModelAsset = call.argument<String>("householdModelAsset")
                executor.execute { loadModel(modelAsset, householdModelAsset, result) }
            }
    ...
    private fun loadModel(
        ...
        try {
            ...
            // Warmup primary interpreter
            candidate.run(inputBuffer!!.apply { clear() }, outputBuffer!!.apply { clear() })

            // Load optional household model
            if (!householdAssetPath.isNullOrBlank()) {
                try {
                    val houseCandidate = Interpreter(
                        loadModelFile(householdAssetPath),
                        Interpreter.Options().apply { numThreads = 2 },
                    )
                    val houseOutputTensor = houseCandidate.getOutputTensor(0)
                    householdInterpreter = houseCandidate
                    householdOutputShape = houseOutputTensor.shape()
                    householdOutputType = houseOutputTensor.dataType()
                    householdOutputScale = houseOutputTensor.quantizationParams().scale
                    householdOutputZeroPoint = houseOutputTensor.quantizationParams().zeroPoint
                    householdOutputBuffer = directBuffer(houseOutputTensor.numBytes())
                    houseCandidate.run(
                        inputBuffer!!.apply { clear() },
                        householdOutputBuffer!!.apply { clear() },
                    )
                } catch (e: Throwable) {
                    householdInterpreter = null
                }
            }
            ...
            success(
                ...
                mapOf(
                    ...
                    "householdLoaded" to (householdInterpreter != null),
                    "householdOutputShape" to householdOutputShape.toList(),
    ...
    private fun runInference(
        ...
        try {
            ...
            val houseInterpreter = householdInterpreter
            val houseOutput = householdOutputBuffer
            var houseValues: FloatArray? = null
            if (houseInterpreter != null && houseOutput != null) {
                houseOutput.clear()
                houseInterpreter.run(inputBuffer!!.apply { rewind() }, houseOutput)
                houseValues = readOutput(
                    houseOutput,
                    householdOutputShape,
                    householdOutputType,
                    householdOutputZeroPoint,
                    householdOutputScale,
                )
            }

            val inferenceMillis = (System.nanoTime() - start) / 1_000_000
            val outputValues = readOutput(
                output,
                outputShape,
                outputType,
                outputZeroPoint,
            ...
            if (houseValues != null) {
                resultMap["householdOutput"] = houseValues
                resultMap["householdShape"] = householdOutputShape.toList()
            }

            success(result, resultMap)
        } catch (error: Throwable) {
    ...
    private fun readOutput(
        ...
        return when (type) {
            ...
            DataType.UINT8 -> FloatArray(count) {
                ((unsigned(buffer.get()) - zeroPoint) * scale)
            }

            DataType.INT8 -> FloatArray(count) {
                ((buffer.get().toInt() - zeroPoint) * scale)
            }
    ...
    private fun releaseInterpreter() {
        interpreter?.close()
        interpreter = null
        householdInterpreter?.close()
        householdInterpreter = null
        inputShape = intArrayOf()
        outputShape = intArrayOf()
        householdOutputShape = intArrayOf()
        inputBuffer = null
        outputBuffer = null
        householdOutputBuffer = null
    }
